    ...
    <div className="sm:hidden hidden md:hidden lg:flex justify-center items-center">
      <div className="w-[90%] pt-[110px] pb-[70px] md:p-24 md:pt-[140px]">
        <div className="text-center flex flex-col gap-[1rem]">
          <h1 className="md:text-start leading-[120%] lg:w-[80%] md:w-[80%] text-black font-semibold font-firansans lg:text-[60px] md:text-[55px] text-[36px]">
          Drive instant traffic with smart Pay-Per-Click strategies.          </h1>
          <p className="text-start md:text-[21px] lg:text-[20px] md:w-[70%] lg:w-[70%]">
          The Smooth Email method allows you to easily increase organic traffic and improve your online presence.          </p>
        </div>
        ...
        <div className="pt-7 flex gap-4">
          {slides.map((slide, index) => (
            <button
              ...
              className={`px-6 p-2 text-[16px] border ${
        ...
        {/* Slider Section */}
        <div className="overflow-hidden lg:w-[77.5%] md:w-[71%] lg:h-[90%] md:h-[90%] w-[90%] relative mt-10">
          <div
            className="flex gap-14"
            style={{
              transform: `translateX(-${currentSlide * 10}%)`,
              width: `${slides.length * 175}%`,
            }}
          >
            {slides.map((slide) => (
              <div
                key={slide.id}
                className={`rounded-[80px] lg:p-[60px] md:p-[60px] flex flex-col gap-[6px] md:w-[340px] md:h-[400px] sm:w-[320px] w-[340px] h-[360px] lg:w-[900px] lg:h-[320px] p-10 border`}
